...
export const Navbar: React.FC = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 px-6 py-4">
      <div className="max-w-7xl mx-auto flex items-center justify-between glass-light rounded-full px-8 py-4">
          ...
          >
            {navLinks.map((link) => (
              <Link
                ...
                className={`text-lg uppercase tracking-widest ${location.pathname === link.path ? 'text-yellow-600' : ''}`}
              >
                {link.name}
              </Link>
            ))}
            ...
            <button onClick={toggleTheme} className="uppercase tracking-widest flex items-center gap-2">
